...
On Windows replace `.venv/bin/python` with `.venv\Scripts\python.exe`. Pick the virtual environment's Python kernel. Installation needs internet; the **offline notebooks themselves do not**; P02 explicitly downloads a public fMRI dataset. Each core notebook is standalone and should be run from top to bottom. Within a section, run its cells in order. GitHub renders notebook text, code, and saved offline outputs without installation. The third_party notebooks retain their own environments and are not included in this execution guarantee.
...
This runs the original offline notebooks in fresh Jupyter kernels. It skips the network project and all upstream reference notebooks. To opt into the public SPM data download and real-run GLM:

```sh
.venv/bin/python scripts/validate_notebooks.py --include-network --match 02_real_fmri_glm
```

Generated data, outputs, environments, and execution copies are ignored by Git. Validation records explain what passed; they do not certify biological conclusions. Start with [F01](../notebooks/00_foundations/01_learning_contract.ipynb) and follow the [study sequence](STUDY_PLAN.md), rather than running dozens of lessons without understanding their outputs.
